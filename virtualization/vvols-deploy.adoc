...
| * 暗号化 * | はい、いいえ、任意 | 7.2以降 | 暗号化されたFlexVolボリュームを選択または作成します。ONTAP ライセンスが必要です。 
...
| * 階層化ポリシー * | [任意]、[なし]、[スナップショット]、[自動] | 7.2以降 | FabricPoolに使用します。AFFまたはASAとONTAP 9.4以降が必要です。NetApp StorageGRID のようなオンプレミスのS3解決策 を使用しないかぎり、Snapshotのみが推奨されます。 
|===
