...
. 環境に適したリージョン（us-east-1 [N.Virginia]など）を選択し、認証方法（[Assume Role]や[AWS Keys]など）を選択します。この検証では、AWSキーを使用します。
...
. 新しい作業環境を作成し、クラウドプロバイダを選択して、Cloud Volumes ONTAPインスタンスのタイプ（シングルCVO、HA、Amazon FSx ONTAP for ONTAPなど）を選択します。
. Cloud Volumes ONTAP クラスタ名やクレデンシャルなどの詳細を入力します。この検証では、というCloud Volumes ONTAPインスタンスを作成しまし `svm_sagemaker_cvo_sn1`た。
...
. 特定のサイズ、許可するプロトコル、およびエクスポートオプションを指定してボリュームを作成します。この検証では、というボリュームを作成しまし `vol1`た。
...
+
.. この検証では、と `vol1`という名前のSVMを作成しまし `svm_sagemaker_cvo_sn1`た。
...
....
. 次の手順では(`dbpedia_csv`、データ）がs3バケットから機械学習で使用されるJupyter Notebookインスタンスにダウンロードされます `ontapbucket1`。
+
....
...
....
+
出力には、AWS SageMaker機械学習検証のデータとして使用されるバケット内のファイルとフォルダが一覧表示され `ontapbucket1`ます。
...
....
. データ前処理フェーズを開始して、トレーニングデータをスペース区切りのトークン化されたテキスト形式に前処理します。この形式は、BlazingTextアルゴリズムとnltkライブラリによって使用され、DBPediaデータセットから入力文をトークン化します。nltkトークナイザーおよびその他のライブラリをダウンロードします。並列で各データインスタンスに適用されるは、 `transform_instance`Pythonマルチプロセッシングモジュールを使用します。
+
....
...
Wall time: 1.32 s
....
. アーティファクトがアルゴリズムのトレーニングジョブの出力になるように、モデルアーティファクトがロードされるS3に出力場所を設定します。トレーニングジョブを起動するオブジェクトを作成し `sageMaker.estimator.Estimator`ます。
+
....
...
....
. リソース構成とハイパーパラメータを使用してSageMakerを定義し `Estrimator`、c4.4xlargeインスタンスの監視モードを使用してDBPediaデータセットのテキスト分類をトレーニングします。
+
....
...
....
. データチャネルとアルゴリズム間のハンドシェイクを準備します。これを行うには、データチャネルからオブジェクトを作成し `sagemaker.session.s3_input`、アルゴリズムが使用するディクショナリに保持します。
+
....
...
....
. ジョブが完了すると、[Job Complete]メッセージが表示されます。トレーニング済みモデルは、試算ツールでとして設定されたS3バケットにあり `output_path`ます。
+
....
...
....
. デフォルトでは、モデルは最も高い確率で1つの予測を返します。上位の予測を取得するには `k`、構成ファイルでを設定し `k`ます。
+
....
