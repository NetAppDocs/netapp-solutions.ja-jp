...
[role="lead"]
以下の3つのPythonスクリプトは、テストした3つの主なユースケースに対応しています。まず第一に `sentiment_analysis_sparknlp.py`
...
....
2番目のスクリプトはです `keras_spark_horovod_rossmann_estimator.py`。
...
if __name__ == '__main__':
    ...
    spark.stop()
....
3番目のスクリプトはです `run_classification_criteo_spark.py`。
