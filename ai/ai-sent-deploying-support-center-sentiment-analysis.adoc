...
を使用します https://["NetApp DataOps ツールキット"^]、次の手順を実行します。
...
セットアップするには https://["NVIDIA NGC"^]、次の手順を実行します。
...
Linux ベースではないオペレーティングシステムについては、を参照してください https://["こちらをご覧ください"^]。



== NVIDIA Rivea サーバ：センタ心理分析をサポートします

セットアップするには https://["NVIDIA RIVA"^]、次の手順を実行します。
...
. Riva クライアント内で、オーディオ処理ライブラリをインストールします（ https://["FFmpeg"^])
+
....
apt-get install ffmpeg
....
. を起動します https://["Jupyter"^] サーバ
...
. を準備してアクティブ化します https://["仮想環境"^] TAO ツールキット用。
. をインストールします https://["必須パッケージ"^]。
...
を使用してください https://["Rivea の Tao ツールキットモデル"^]、次の手順を実行します。
