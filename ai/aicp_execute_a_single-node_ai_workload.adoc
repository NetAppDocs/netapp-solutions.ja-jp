...
[role="lead"]
Kubernetes クラスタでシングルノードの AI ジョブと ML ジョブを実行するには、導入ジャンプホストから次のタスクを実行します。Trident を使用すると、数ペタバイトのデータが含まれる可能性のあるデータボリュームをすばやく簡単に作成し、 Kubernetes のワークロードからアクセスできます。Kubernetes ポッド内からこのようなデータボリュームにアクセスできるようにするには、で作成した PVC の 1 つなど、 PVC を指定します link:aicp_import_an_existing_volume.html["既存のボリュームをインポートします"]をクリックします。このステップは Kubernetes ネイティブの運用であり、ネットアップの専門知識は不要です。
...
+
また、必要なストレージ帯域幅を提供するために、必要なトレーニングデータ（セクションの例でインポートしたボリューム）が含まれているボリューム link:aicp_import_an_existing_volume.html["既存のボリュームをインポートします"]）は、このジョブが作成するポッド内に 2 回マウントされます。以下のジョブ定義の強調表示された行を参照してください。を参照してください link:aicp_netapp_trident_deployment_and_configuration_overview.html["Trident の導入と設定の概要"]の手順 1 で、同じデータボリュームを複数回マウントする理由を確認してください。必要なマウントの数は、特定のジョブで必要とされる帯域幅によって異なります。

+
の例で作成したボリューム セクション link:aicp_provision_a_new_volume.html["新しいボリュームをプロビジョニングします"] ポッドにも取り付けられます。これらのボリュームは、 PVC の名前を使用してジョブ定義内で参照されます。Kubernetes ジョブの詳細については、を参照してください https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/["Kubernetes の公式ドキュメント"^]。
