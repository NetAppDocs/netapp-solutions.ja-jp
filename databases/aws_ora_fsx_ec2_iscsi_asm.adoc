...
[%collapsible]
====
前提条件をプロビジョニングした状態で、rootユーザとしてEC2インスタンスにログインし、Oracleインストール用のLinuxカーネルを設定します。
...
[%collapsible]
====
FSxコンソールから3つのボリュームをプロビジョニングして、Oracleデータベースのバイナリファイル、データファイル、ログファイルをホストします。
...
. EC2-userとしてSSH経由でEC2インスタンスにログインします。次に、SSHキーとEC2インスタンスのIPアドレスに変更します。
+
....
...
....
. デバイスの所有権をに変更します `oracle:oinstall`。
+
....
...
....
. Oracleユーザとしてを起動します `gridSetup.sh` グリッドインフラのインストールに使用します。
+
....
...
....
. 「[INS-43080] Some of the configuration assistants failed, were cancelled or skipped」というメッセージが表示された場合は、Oracleユーザとして次のコマンドを実行し、データディスクグループリソースが作成されたかどうかを確認します。
+
....
bin/crsctl stat res -t
....
. データディスクグループが作成されていない場合は、multipathdをrootユーザとしてリロードします。
+
....
systemctl restart multipathd
....
. グリッドのインストール時にデータディスクグループが作成されていない場合は、Oracleユーザとしてデータディスクグループを作成します。
+
....
...
....
. ソフトウェアのみのデータベースのサイレントインストールを実行します。
+
....
 ./runInstaller -applyRU /tmp/archive/34765931/ -silent -ignorePrereqFailure -responseFile /tmp/archive/dbinstall.rsp
....
. を実行します `root.sh` ソフトウェアのみのインストール後にスクリプトを作成します。
+
....
/u01/app/oracle/product/19.0.0/db1/root.sh
....
. を作成します `dbca.rsp` 次のエントリを含むファイル：
+
....
