...
このドキュメントでは、ASAシステムで構築されたSAN環境に、Ansibleによる自動化を使用してOracleデータベースを簡単に導入する方法について説明します。Oracleデータベースは、スタンドアロンの再起動構成で導入され、データアクセス用にiSCSIプロトコル、ASAストレージアレイ上のストレージ管理用にOracle ASMを使用します。また、開発とテストや、NetApp ASAシステムでのストレージ効率に優れたデータベース運用のために、NetApp SnapCenter UIツールを使用してOracleデータベースをバックアップ、リストア、クローニングする方法についても説明します。
...
image::automation_ora_asa_iscsi_archit.png[このイメージは、iSCSIとASMを使用したAWSパブリッククラウドでのOracle導入構成の詳細な図を示しています。]
...
[%collapsible]
====
TR-4979を参照 link:https://docs.netapp.com/us-en/netapp-solutions/databases/aws_ora_fsx_vmc_guestmount.html#oracle-backup-restore-and-clone-with-snapcenter["ゲストマウント型FSx ONTAPにより、VMware Cloud on AWS上のシンプルで自己管理型のOracleを実現"^] セクション。 `Oracle backup, restore, and clone with SnapCenter` SnapCenterをセットアップし、データベースのバックアップ、リストア、クローニングのワークフローを実行する方法の詳細については、を参照してください。
