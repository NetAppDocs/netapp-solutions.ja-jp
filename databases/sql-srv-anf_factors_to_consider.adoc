...
あらゆる導入同様、 VM とストレージをテストすることが重要です。ストレージの検証には、HammerDB、Appploader、、、または適切な読み取り/書き込みの組み合わせを持つ任意のカスタムスクリプトまたはfioなどのツールを https://github.com/NetApp/SQL_Storage_Benchmark["SQL Server Storage Benchmark （ SB ）ツール"^]使用する必要があります。ただし、 SQL Server のワークロードのほとんどは、ビジー状態の OLTP ワークロードでも、読み取りが 80~90% 、書き込みが 10~20% 近くになることに注意してください。
