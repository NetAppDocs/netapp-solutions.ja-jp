...
*エアギャップ（論理）*- ONTAPを使用すると、NetAppストレージを論理的なエアギャップのあるサイバーボールトとして構成できます。link:https://docs.netapp.com/us-en/ontap/snaplock/commit-snapshot-copies-worm-concept.html["SnapLock ComplianceによるSnapshotコピーのWORM保護"^]
