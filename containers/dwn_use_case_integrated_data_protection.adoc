...
このユースケースの検証に使用されたアプリケーションは、Webベースのフロントエンドを備えたEコマースソリューション、検索と分析機能のためのElasticsearchインスタンス、すべてのショッピングインベントリとトランザクションの詳細を追跡するMariaDBデータベースでした https://magento.com/["Magento"^]。このコンテナ化されたアプリケーションは、Red Hat OpenShiftクラスタにインストールされました。アプリケーション内のすべてのポッドで、永続ボリュームを使用してデータを格納しました。永続ボリュームは、コンテナストレージインターフェイスに準拠したKubernetes向けストレージオーケストレーションツールであるNetApp Tridentによって自動的に作成され、NetAppストレージシステムでストレージをプロビジョニングできます。さらに、Astra Control Centerのアプリケーション保護機能を利用するために、問題のアプリケーションはAstra Controlによって管理されました。Astra Controlは、永続的ボリュームに保存されたデータとともに、アプリケーションの状態を保存したアプリケーションのバックアップをトリガするために使用されました。を使用して https://github.com/NetApp/netapp-astra-toolkits["NetApp Astra Control Python SDK"^]アプリケーションのバックアップをトリガーするプロセスを自動化し、CI / CDパイプラインに導入しました。このパイプラインは[https://www.jenkins.io/["Jenkins"^]、一般的なCI/CDツールである]を使用して作成および実行され、アプリケーションのビルド、保護、デプロイのフローを自動化しました。
...
. NetApp TridentをOpenShiftにインストールし、NetApp ONTAPシステムへのバックエンドを構成
