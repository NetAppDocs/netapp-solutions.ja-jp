...
NOTE: Astra Control Center によって管理されるすべての Red Hat OpenShift クラスタは、管理対象クラスタにインストールされたエージェントがそのレジストリからイメージをプルするためにインストールに使用されたイメージレジストリにアクセスできる必要があります。
...
link:rh-os-n_astra_applications.html["次のセクションでは、保護するアプリケーションを選択してください。ネットアップで Red Hat OpenShift を実装しています。"]
