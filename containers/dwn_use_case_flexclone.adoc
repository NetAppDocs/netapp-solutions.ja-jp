...
. ストレージバックエンドがNetApp Tridentで構成されたNetApp ONTAPシステム。
. Ansible 2.9以降
...
これらの各環境は、NetApp Astra Controlによって管理されます。現在の永続ボリュームは、NetApp Tridentが提供するNFSバックエンドを備えたNetApp ONTAPストレージシステムに格納されています。
