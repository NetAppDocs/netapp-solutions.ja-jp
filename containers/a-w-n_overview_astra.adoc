...
Astra Control CenterをサポートするためのTridentのインストールと設定については、を参照してくださいlink:rh-os-n_overview_trident.html["本ドキュメント"^]。
...
----
. ログをチェックして `acc-operator-controller-manager`、インストールが完了したことを確認してください。
+
[listing]
----
