...
---
= NetApp Astra コントロールセンターの概要：ネットアップを使用した Red Hat OpenShift
:hardbreaks:
:nofooter: 
:icons: font
:linkattrs: 
...
NetApp Astra Control Center は、ネットアップの信頼できるデータ保護テクノロジを基盤とするオンプレミス環境に導入された、ステートフル Kubernetes ワークロード向けの豊富なストレージおよびアプリケーション対応データ管理サービスを提供します。

image:redhat_openshift_image44.png["エラー：グラフィックイメージがありません"]

NetApp Astra Control Center は、 Astra Trident ストレージオーケストレーションツールを導入し、ストレージクラスを構成し、 NetApp ONTAP ストレージシステムにストレージバックエンドを接続した Red Hat OpenShift クラスタにインストールできます。

Astra Control Center をサポートする Astra Trident のインストールと設定については、本ドキュメントの link:rh-os-n_overview_trident.html["こちらをご覧ください"]。

クラウド接続環境では、 Cloud Insights を使用して高度なモニタリングとテレメトリを提供します。Cloud Insights 接続がない場合は、限定的な（ 7 日間の指標）監視と計測データが使用可能になり、オープン指標エンドポイントを介して Kubernetes の標準の監視ツール（ Prometheus / Grafana ）にエクスポートされます。

Astra Control Center は、 NetApp AutoSupport / Active IQ エコシステムに完全に統合されており、ユーザをサポートし、トラブルシューティングを支援し、使用状況の統計を表示します。

支払い済みバージョンの Astra Control Center に加え、 90 日間の評価ライセンスがお客様に提供されます。評価版は、 E メールとコミュニティ（ Slack チャンネル）を通じてサポートされます。お客様は、これらの記事やその他のナレッジベース記事、および製品サポートダッシュボードから入手可能なドキュメントにアクセスできます。

ネットアップアストラコントロールセンターの利用を開始するには、にアクセスしてください https://cloud.netapp.com/astra[]。



== Astra Control Center のインストールの前提条件

. 1 つ以上の Red Hat OpenShift クラスタ。
+

NOTE: 現在、バージョン 4.6 EUS および 4.7 がサポートされています。

. 各 Red Hat OpenShift クラスタに Astra Trident をインストールして設定しておく必要があります。
. ONTAP 9.5 以降を実行している NetApp ONTAP ストレージシステムが 1 つ以上必要です。
+

NOTE: サイトに各 OpenShift インストールを実装し、永続的ストレージ専用の SVM を用意することがベストプラクティスです。マルチサイト環境では、追加のストレージシステムが必要になります。
...
NOTE: Docker または Podman 環境は、バージョン 20.10 以上である必要があります。



== Astra Control Center をインストールします

. ネットアップサポートサイトにログインし、最新バージョンの NetApp Astra Control Center をダウンロードします。この処理を実行するには、ライセンスをネットアップアカウントに接続する必要があります。tarball をダウンロードしたら、 admin ワークステーションに転送します。
+

NOTE: Astra Control の試用版ライセンスの使用を開始するには、サイトにアクセスしてください https://cloud.netapp.com/astra-register["こちらをご覧ください"^]
...
----
. インストールを開始する前に、 Astra Control Center イメージをイメージレジストリにプッシュする必要があります。
+
...
----
. シェルスクリプトファイルを作成し、その中に以下の内容を貼り付けます。
+
[listing]
----
...
NOTE: レジストリに信頼されていない証明書を使用している場合は、シェルスクリプトを編集し、 podman push コマンドに --tls-verify=false を使用します
...
NOTE: レジストリに信頼されていない証明書を使用している場合は、シェルスクリプトを編集して、 Docker Push コマンドの -`d Occker push $registry/$(echo $astraatImage|seds's/^[^\\/]\\/')--tls-verify=false`) を使用します
...
--
====
. 次の手順では、イメージレジストリ TLS 証明書を OpenShift ノードにアップロードします。そのためには、 TLS 証明書を使用して OpenShift -config ネームスペースに ConfigMap を作成し、クラスタイメージ設定にパッチを適用して証明書を信頼できるようにします。
+
[listing]
----
...
NOTE: ルートを含む入力オペレータからのデフォルト TLS 証明書で OpenShift 内部レジストリを使用している場合でも、上記の手順に従って、証明書をルートホスト名にパッチする必要があります。入力オペレータから証明書を抽出するには、コマンド -`oc extract secret/router-ca --keys=tls.crt-n OpenShift ingress-operator` を使用します

. Astra Control Center Operator をインストールするための名前空間「 acc-operator-system 」を作成します。
+
[listing]
----
[netapp-user@rhel7 ~]$ oc create ns acc-operator-system
----
. イメージレジストリにログインするためのクレデンシャルを持つシークレットを 'acc-operator-system' 名前空間に作成します
+
[listing]
----
...
----
. Astra Control Center Operator CR 'Astra_control_center_deployment.yaml ' を編集します Astra Control Center は ' すべてのリソースを配備しますオペレータ CR で、「 accoperator-controller-manager 」の展開定義を検索し、レジストリの FQDN と、イメージをレジストリにプッシュする際に指定した組織名を入力します（この例では astra-registry.apps.ocp-vmw.cie.netapp.com/netapp-astra ）。 テキスト「 [ あなたの .registry.goes .here] 」を置き換え、先ほど作成した秘密の名前を入力します。オペレータのその他の詳細を確認し、保存して閉じます。
+
[listing]
----
...
----
. その名前空間でイメージレジストリにアクセスするためのシークレットを作成します。
+
[listing]
----
...
secret/astra-registry-cred created
----
. 次の手順では、 Astra Control Center CRD ファイル「 Astra_control_center_min YAML 」を編集して、 FQDN 、イメージレジストリの詳細、管理者の電子メールアドレス、およびその他の詳細を入力します。
+
[listing]
----
...
NOTE: 上記のファイル「 Astra_control_center_min YAML 」は、 Astra Control Center CRD の最小バージョンです。PVC 作成時のデフォルト以外のストレージクラスの定義やメール通知用の SMTP 詳細の提供など、より詳細な制御を使用して CRD を作成する場合は、ファイル「 Astra_control_center.yaml 」を編集し、これらの詳細を入力して CRD を作成します。
...
NOTE: 次のメッセージが表示され、 Astra Control Center のインストールが正常に完了したことが示されます
...
----
. Astra Control Center にログインするためのユーザ名は、 CRD ファイルに入力された管理者の電子メールアドレスで、パスワードは Astra Control Center UUID に付加された文字列「 ACC-」 です。次のコマンド–を実行します
+
[listing]
----
...
. 次に、 CRD ファイルで指定した FQDN にアクセスして、 Astra Control Center GUI にログインします。
+
...
. Astra Control Center にユーザーを追加する場合は、「アカウント」 -> 「ユーザー」と進み、「追加」をクリックしてユーザーの詳細を入力し、「追加」をクリックします。
+
image:redhat_openshift_image89.jpg["Astra Control Center でユーザを作成"]

. Astra Control Center では、すべての機能が動作するためにライセンスが必要です。ライセンスを追加するには、「アカウント」 -> 「ライセンス」に移動し、「ライセンスの追加」をクリックしてライセンスファイルをアップロードします。
+
