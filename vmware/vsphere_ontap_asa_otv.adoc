...
+
image:vmware-asa-image20.png["[Summary]ページ"]
...
+
image:vmware-asa-image27.png["ストレージの選択"]
...
+
image:vmware-asa-image28.png["ストレージの選択"]

. 最後に、概要ページを確認し、*[Finish]*をクリックしてVMを作成します。


====
つまり、NetApp ONTAPツールを使用すると、ONTAPストレージシステムにVVOLデータストアを作成するプロセスが自動化されます。ストレージ機能プロファイルは、データストアの作成に使用するストレージシステムを定義するだけでなく、VMDK単位で実装できるQoSポリシーも定義します。VVOLは、シンプルなストレージ管理パラダイムを提供し、NetAppとVMwareの緊密な統合により、仮想環境を効率的かつきめ細かく制御するための実用的なソリューションです。
