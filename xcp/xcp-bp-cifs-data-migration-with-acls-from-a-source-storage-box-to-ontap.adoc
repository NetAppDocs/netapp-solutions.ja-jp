...
NOTE: データアグリゲートがない場合は、コマンドを使用して新しいアグリゲートを作成し `storage aggr create`ます。
...
....
. XCP Windowsクライアントホストシステムでコマンドを実行して、ソースノードのSMBエクスポートを照会します `xcp show`。
+
....
...
....
. ターゲットのONTAPシステムで、および `fallback-group`Argumentsパスの値として指定する必要があるローカルユーザとローカルグループの名前のリストを取得します `fallback-user`。
+
....
...
5 entries were displayed
....
. ACLを使用してCIFSデータをソースからターゲットに移行するには、オプションと `–fallback-user/group`オプションを指定してコマンドを `-acl`実行し `xcp copy`ます。
+
オプションには `fallback-user/group`、Active Directoryまたはローカルユーザ/グループからターゲットシステムへの任意のユーザまたはグループを指定します。
...
C:\WRSHDNT>
....
. エラーメッセージが表示された `ERROR failed to obtain fallback security principal`場合は `xcp copy`、hostsファイルに保存先のボックスを追加し(`C:\Windows\System32\drivers\etc\hosts`ます）。
+
...
....
. hostsファイルに宛先ボックスエントリを追加してもエラーメッセージが表示される場合は、 `ERROR failed to obtain fallback security principal`ターゲットシステムにユーザー/グループが存在しません。
+
....
...
C:\WRSHDNT>
....
. ACLを使用して（ルートフォルダの有無にかかわらず）CIFSデータを移行する場合に使用します `xcp copy`。
+
