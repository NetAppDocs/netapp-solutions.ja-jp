...
NOTE: XCP ファイル分析のアーキテクチャの概要、統計情報ビューなどの GUI ベースのダッシュボードビュー、およびファイル分散ビューの詳細については、を参照してください https://blog.netapp.com/xcp-data-migration-software["NetApp XCP 1.6 が、オープンファイル分析とインフラストラクチャの向上を実現します"^] ブログ

XCP 1.6 では、カスタマイズされたグラフ用の GUI に制限があります。必要なグラフを作成するには 'XCP scan コマンドを実行し ' フィルタを一致させて CLI を活用します次の例を参照してください。
