...
summary: この解決策は、特定の日付に基づいてデータをコピーする必要があるお客様を対象としています。 
---
= データの特定の日付ベースのスキャンおよびコピー
:hardbreaks:
:nofooter: 
:icons: font
:linkattrs: 
...
link:xcp-bp-duplicate-files.html["前へ：ファイルを複製します。"]

この解決策は、特定の日付に基づいてデータをコピーする必要があるお客様を対象としています。次の情報を確認します。
...
c:\XCP>
....
link:xcp-bp-creating-a-csv-file-from-smb-cifs-share.html["次の手順： SMB / CIFS 共有からの CSV ファイルの作成"]
