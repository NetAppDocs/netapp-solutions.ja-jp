...
link:xcp-bp-creating-a-csv-file-from-smb-cifs-share.html["前の手順： SMB / CIFS 共有からの CSV ファイルの作成"]
...
....
. ターゲット SVM から FCP 、 iSCSI 、 NDMP 、 CIDS の各プロトコルを削除します。
+
....
...
....
. データ LIF を作成して NFS クライアントの要求に対応します。
+
....
...
....
. このマウントポイントに、ターゲットの NFSv3 エクスポートボリュームをマウントします。
+
...
....
. XCP Linux クライアントホストシステムで「 XCP show 」コマンドを実行して、ソースの 7-Mode NFSv3 エクスポートを照会します。
+
....
...
....
. コピーが完了したら、ソースとデスティネーションの NFSv3 エクスポートに同一のデータがあることを確認します。「 XCP verify 」コマンドを実行します。
+
....
...
. カットオーバーの前後に、もう一度「ライフル」を実行します。ソースに新規または更新されたデータがある場合は、差分更新を実行します。「 XCP sync 」コマンドを実行します。
+
....
...
....
. 以前に中断されたコピー操作を再開するには 'XCP RESUME コマンドを実行します
+
....
...
NOTE: ソースの 7-Mode ボリュームがクライアントシステムにエクスポートされてマウントされ、 XCP が Linux システムにすでにインストールされていることを前提としています。Snapshot コピーはボリュームのポイントインタイムイメージであり、前回の Snapshot コピー作成後の差分変更を記録します。7-Mode システムをソースとして「 snap 」オプションを使用します。
...
. カットオーバーの前後に、もう一度「ライフル」を実行します。ソースに新規または更新されたデータがある場合は、差分更新を実行します。増分変更がある場合は、これらの変更の新しい Snapshot コピーを作成し、そのスナップショットパスを sync 操作のための「 -snap' 」オプションで渡します。
+
...
[root@localhost linux]#
....
. XCP Linux クライアント・ホスト・システムで XCP show コマンドを実行して、ソース NFSv4 エクスポートを照会します。
+
....
...
....
. 「 copy 」が完了したら、ソースおよびデスティネーションの NFSv4 エクスポートに同一のデータがあることを確認します。「 XCP verify 」コマンドを実行します。
+
....
...
. 以前に中断された「 copy 」操作を再開するには、「 XCP resume 」コマンドを実行します。
+
....
...
NOTE: データ・アグリゲートが存在しない場合は 'storage 'aggr create ' コマンドを使用して新しいアグリゲートを作成します
...
link:xcp-bp-cifs-data-migration-with-acls-from-a-source-storage-box-to-ontap.html["次の例： ACL を使用した、ソースストレージボックスから ONTAP への CIFS データの移行"]
