...
* 既存のGoogle Cloud VMware Engine Software-Defined Data Center（SDDC）を使用するか、このツールを使用してプライベートクラウドを作成します https://docs.netapp.com/us-en/netapp-solutions/ehc/gcp/gcp-setup.html["ネットアップのリンク"^] またはこれ https://cloud.google.com/vmware-engine/docs/create-private-cloud["Googleリンク"^]。
