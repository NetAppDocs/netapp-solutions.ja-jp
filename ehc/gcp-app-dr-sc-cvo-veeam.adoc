...
image::dr-cvo-gcve-image1.png[DRのCVOグループイメージ1]
...
[%collapsible]
====
最初のステップは、Google CloudでCloud Volumes ONTAPを設定することです（link:gcp-guest.html#gcp-cvo["CVOを確認して"^]）をクリックし、必要なボリュームを、必要な頻度とSnapshotの保持を使用してCloud Volumes ONTAP にレプリケートします。

image::dr-cvo-gcve-image2.png[DRのCVOグループイメージ2]
