...
+
image::avs-register-create-pc-1.png[AVSレジスタの作成PC 1]
...
image::avs-register-create-pc-4.png[AVSレジスタの作成PC 4]
...
+
image::azure-connect-gateway-1.png[Azure Connectゲートウェイ1]

. 新しい VNet の情報を入力または更新し、 OK を選択します。


image::azure-connect-gateway-2.png[Azure Connectゲートウェイ2]
...
image::azure-validate-network-1.png[Azure Validateネットワーク1]

仮想マシンをプロビジョニングしたら、 Connect オプションを使用して RDP にアクセスします。

image::azure-validate-network-2.png[Azure Validateネットワーク2]
...
image::azure-validate-network-3.png[Azure Validateネットワーク3]

Windows 仮想マシンでブラウザを開き、 vCenter Web Client の URL にアクセスします (`"https://10.21.0.2/"`）にアクセスし、adminユーザ名に「* cloudadmin@vsphere.loca l *」と入力して、コピーしたパスワードを貼り付けます。同様に、 Web クライアントの URL を使用して NSX Manager にアクセスすることもできます (`"https://10.21.0.3/"`）をクリックし、adminユーザ名を使用してコピーしたパスワードを貼り付け、新しいセグメントを作成するか、既存の階層ゲートウェイを変更します。


NOTE: Web クライアントの URL は、プロビジョニングされる SDDC ごとに異なります。

image::azure-validate-network-4.png[Azure Validateネットワーク4]

image::azure-validate-network-5.png[Azureでネットワークを検証5]
