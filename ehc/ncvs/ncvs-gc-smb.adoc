...
リージョンとサービスタイプごとに許可されるActive Directory接続は1つだけです。
...
| LM互換性レベル | Active Directory接続でサポートされている認証ダイアレクトのレベル。「」を参照してください authentication dialects」を参照してください。 | NTLMv2 - krb | いいえ 


| 受信CIFSトラフィックにSMB暗号化を要求します | すべての共有でSMB暗号化が必要です。これはCloud Volumes Service では使用されません。代わりに、ボリューム単位で暗号化を設定します（「」を参照） share security features」）をクリックします。 | いいえ | いいえ 
