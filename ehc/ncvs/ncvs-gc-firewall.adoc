...
Cloud Volumes Service は、「gcloud compute address」コマンドで割り当てられたCIDR範囲のIPアドレスで到達します https://cloud.google.com/architecture/partners/netapp-cloud-volumes/setting-up-private-services-access?hl=en_US["Cloud Volumes Service への参加"^]。このCIDRをソースアドレスとして使用して、Active Directoryドメインコントローラへのインバウンドファイアウォールを設定できます。
