...
NOTE: Snapshotラベルは、セカンダリONTAPクラスタにレプリケートされたSnapVaultコピーの保持期間を指定したポリシーを作成する場合に役立ちます。BlueXPのバックアップおよびリストアでSCVを使用する場合は、[Snapshot label]フィールドを空白にするか、<u> match </u>に指定したラベルを指定する必要があります。
