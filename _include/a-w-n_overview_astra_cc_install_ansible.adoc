...
. Ansibleプレイブックを使用してAstra Control Centerを導入するには、AnsibleがインストールされたUbuntu / RHELマシンが必要です。Ubuntuおよびlink:../automation/automation_rhel_centos_setup.html["ここをクリック"]RHELの場合は説明されている手順に従いますlink:../automation/automation_ubuntu_debian_setup.html["ここをクリック"]。
