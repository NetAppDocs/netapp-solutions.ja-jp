...
NOTE: ユーザーを使用してプライベートレジストリにログインしている場合は `kubeadmin`、パスワードの代わりにトークンを使用してください- `docker login -u ocp-user -p token astra-registry.apps.ocp-vmw.cie.netapp.com`。
...
----
. クレデンシャルを使用してシークレットを作成し、ネームスペースのイメージレジストリにログインし `netapp-acc-operator`ます。
+
[listing]
----
...
. タイルを選択し `netapp-acc-operator`、[インストール]をクリックします。
+
...
. フォームフィールドに入力し `Create AstraControlCenter`、[作成]をクリックします。
+
